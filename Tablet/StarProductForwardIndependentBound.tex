...
Moreover, a root-to-depth-\(k\) path in \(\mathcal T\) is exactly an ordered
forward independent \(k\)-tuple in \(D^*(G)\), by the construction of
\(\mathcal T\) and \(\noderef{ForwardIndependentTuple}\); therefore the number
of such paths is \(\fwi_k(D^*(G))\) by
\(\noderef{ForwardIndependentTupleCount}\).  Applying
