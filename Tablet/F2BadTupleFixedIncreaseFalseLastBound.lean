...
-- [TABLET NODE: F2BadTupleFixedIncreaseFalseLastBound]

theorem F2BadTupleFixedIncreaseFalseLastBound (p m : ℕ)
    (z : Fin (m + 1) → Bool) (hzlast : z (Fin.last m) = false) :
...
-- BODY
  ...
  have hweight_zp : BinarySequenceWeight zp = BinarySequenceWeight z := by
    have h := BinarySequenceWeightSnoc z
    simpa [zp, hzlast] using h.symm
  ...
  have hpref_rank_weight : ∀ pref : Pref,
      F2BadTupleRank p m pref.val m = BinarySequenceWeight z := by
    intro pref
    by_cases hm0 : m = 0
    · subst m
      have hweight_zero : BinarySequenceWeight z = 0 := by
        have hlast0 : z (0 : Fin 1) = false := by
          simpa using hzlast
        simp [BinarySequenceWeight, hlast0]
      exact (F2BadTupleRankZero p 0 pref.val).trans hweight_zero.symm
    · have hmpos : 0 < m := Nat.pos_of_ne_zero hm0
      have hcard :=
        (F2BadTupleRankIncreaseSetCard p m pref.val pref.property.1 hmpos).2
      have hset_card :
          (F2BadTupleRankIncreaseSet p m pref.val).card =
            BinarySequenceWeight zp := by
        simp [F2BadTupleRankIncreaseSet, BinarySequenceWeight, pref.property.2]
      exact hcard.symm.trans (hset_card.trans hweight_zp)
  ...
  have hfiber_le :
    ...
    simpa [Fiber, Vec] using
      F2BadTupleLastPairSpanFiberBound p m (BinarySequenceWeight z)
        pref.val (hpref_rank_weight pref)
  have hsum_const :
      Finset.univ.sum
          (fun _ : Pref =>
            2 ^ BinarySequenceWeight z * 2 ^ (p - BinarySequenceWeight z)) =
  ...
  calc
    ...
    _ ≤
        Finset.univ.sum
          (fun _ : Pref =>
            2 ^ BinarySequenceWeight z * 2 ^ (p - BinarySequenceWeight z)) := hfiber_le
    _ =
        Fintype.card Pref *
          (2 ^ BinarySequenceWeight z * 2 ^ (p - BinarySequenceWeight z)) := hsum_const
    _ =
        F2BadTupleFixedIncreaseCount p m (fun i : Fin m => z i.castSucc) *
          (2 ^ BinarySequenceWeight z * 2 ^ (p - BinarySequenceWeight z)) := by
        rw [hpref_count]
