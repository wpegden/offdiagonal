...
By \(\noderef{F2ForwardIndependentLinearBound}\), after increasing the lower threshold for \(s\) if necessary, there are a finite vertex set \(W\) and a digraph \(D\) on \(W\) such that \(D\) is loopless, \(D\) is \(T_s\)-free, and
\[
  N\coloneqq |W|
  =2^{2s-3}-2^{s-1}-2^{s-2}+1
\]
while, in the notation of \(\noderef{ForwardIndependentTupleCount}\),
\[
...
Apply \(\noderef{DigraphToGraphIndependentSetBound}\) to \(D\), using the \(T_s\)-freeness from \(\noderef{F2ForwardIndependentLinearBound}\) and \(k\ge1\). We obtain a \(K_s\)-free simple graph \(G\) on the same vertex set \(W\) such that, writing \(i_k(G)\) for the independent-set count from \(\noderef{SimpleGraphIndependentSetCount}\),
\[
